...
int tokenfind_integer_batch(const char* src_sequence,
    ...
    for(int i=0; i<n_jobs; i++) {
        ...
        ret = tokenfind_integer_single(src_sequence,
                                       ...
                                       job[i].opt_param);
        if(ret > 0) {
            target = (int*)(base_ptr + job[i].member_offset);
            *target = tmp_result;
            n_success++;
        }
    }
...
int tokenfind_float_batch(const char* src_sequence,
    ...
    for(int i=0; i<n_jobs; i++) {
        ...
        ret = tokenfind_float_single(src_sequence,
                                       ...
                                       &job[i].regex_cache);
        if(ret > 0) {
            target = (float*)(base_ptr + job[i].member_offset);
            *target = tmp_result;
            n_success++;
        }
    }
...
int tokenfind_string_batch(const char* src_sequence,
    ...
    for(int i=0; i<n_jobs; i++) {
        ...
        len = tokenfind_string_single(src_sequence,
                                      ...
                                      &job[i].regex_cache);
        if(len > 0) {
            target = (char*)(base_ptr + job[i].member_offset);
...
int tokenfind_string_single(const char* src_sequence,
    ...
    strncpy(dst_sequence, &src_sequence[match_start], MIN(dst_length, len));
    dst_sequence[MIN(dst_length, len)-1] = 0;   /* NULL termination */
    return MIN(dst_length, len);
}
